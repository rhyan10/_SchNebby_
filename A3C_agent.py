...
class Actor:
    ...
    def compute_loss(self, r_chosen_batch, r_dist_batch, r_log_batch, advantages):
        ...
        for r_chosen_t, r_dist_t, r_log_t, ad in zip(r_chosen_batch, r_dist_batch, r_log_batch, advantages):
            ...
            entropy = - (r_log_t['f']*r_dist_t['f']).sum(-1, keepdim=True)
            ...
            f_policy = - ad * r_log_t['f'] - self.entropy_beta * entropy
            f_policy = f_policy.sum()
            
            r_chosen_t['t'] = torch.unsqueeze(r_chosen_t['t'], -1)
            entropy = - (r_log_t['t']*r_dist_t['t']).sum(-1, keepdim=True)
            entropy = torch.squeeze(entropy)
            ...
            t_policy = - ad * r_log_t['t'] - self.entropy_beta * entropy
        ...
        for epoch in tqdm(range(self.max_epochs)):
            for initial_state in self.dataloader:            
                ...
                for counter in  (range(self.episode_length)):
                    ...
                    atoms, reward, forces, energies = self.env.step(forces, energies, force_index, tangent_index, inputs, atoms, epoch, counter)
                    ...
                    if len(reward_batch) >= self.update_interval or done == True:
                        ...
                        if done == True:
                            reward_sum = np.sum(reward.numpy())
                            logger.info('Reward: '+ str(reward_sum))
                            if reward_sum > self.max_reward:
                                self.max_reward = reward_sum
                                torch.save(self.actor.model, 'logging/actor_model')
                                torch.save(self.critic.model, 'logging/critic_model')                                
                        ...
                        actor_loss = self.actor.loss(r_chosen_batch, r_dist_batch, r_log_batch, advantages)
